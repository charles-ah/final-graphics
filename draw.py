...
def scanline_convert(polygons, i, screen, zbuffer):
   ...
   # color = shade(polygons, i, color)
    ...
    ymid=sorted([a,b,c],key=lol)[1][1]
    xstart=sorted([a,b,c],key=lol)[0][0]
    xmid=sorted([a,b,c],key=lol)[1][0]
    xend=sorted([a,b,c],key=lol)[2][0]
    dy = 1
    ...
    if ymid!=ybot:
        ...
        while y<ytop:
            ...
            if y>=ymid:
                dx1 = float(xend-xmid)/float(ytop-ymid)
                dx0 = float(xend-xstart)/float(ytop-ybot)
